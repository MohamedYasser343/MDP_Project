ITERATIONS = 100  # Increased from 20 for better convergence
...
CONVERGENCE_THRESHOLD = 1e-3  # Convergence threshold for early stopping
converged = False
for iteration in range(ITERATIONS):
    ...
    # Check convergence
    max_delta = max(abs(new_values[s] - values[s]) for s in STATES)
    if iteration % 5 == 0:  # Print progress every 5 iterations
...
if not converged:
    ...
    print(f"Value iteration complete. Final policy computed.")
# print(values.values(), policy.values())  # Commented out - too verbose
